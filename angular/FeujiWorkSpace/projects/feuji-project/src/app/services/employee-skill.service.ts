...
saveSkillCategoryAdmin(newCategory: SkillCategoryBean): Observable<SkillCategoryBean> {
  const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
  return this.http.post<SkillCategoryBean>('http://13.48.82.196:8092/api/referencedetails/save', newCategory, { headers });
}

saveSubSkillCategoryAdmin(newSubCategory: SubSkillCategoryBean): Observable<SubSkillCategoryBean> {
  const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
  return this.http.post<SubSkillCategoryBean>('http://13.48.82.196:8092/api/referencedetails/addSubSkill', newSubCategory, { headers });
}

subSkillAdmin(newSkill: Skill): Observable<Skill> {
  const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
  return this.http.post<Skill>('http://13.48.82.196:8087/api/skill/insert', newSkill, { headers });
}

public getSkillCategoryNames():Observable<any[]>
{

  return this.http.get<any[]>(`http://13.48.82.196:8092/api/referencedetails/getref/Skill Category`)
}


public getTrainingRecommendedEmployees(skillIds:number[]): Observable<any>
{
  return this.http.get<any>(`http://13.48.82.196:8087/api/fetchallemployees/${skillIds}`);
}
  
public updateStatusAdmin(skillId: number, status: number): Observable<any> {
    return this.http.put<any>(`http://13.48.82.196:8087/api/skill/updateStatus?skillId=${skillId}&status=${status}`,{});
}
